...
const elements = {
    ...
    labelsBtn:      document.getElementById('labels-btn'),     // NEW
...
function showInfo(data) {
    ...
    // NEW: Molecular statistics (atom & bond counts from loaded model)
    if (state.viewer) {
        try {
            const model = state.viewer.getModel();
            const atoms = model?.selectedAtoms({}) || [];
            elements.infoAtoms.textContent = atoms.length;
            // Estimate bonds from the model's bond data
            let bondCount = 0;
            for (const atom of atoms) {
                bondCount += (atom.bonds || []).length;
            }
            // Each bond is counted twice (once per atom), so halve it
            elements.infoBonds.textContent = Math.round(bondCount / 2);
        } catch {
            elements.infoAtoms.textContent = '—';
            elements.infoBonds.textContent = '—';
        }
    }
}
